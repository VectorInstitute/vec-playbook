#!/bin/bash
#SBATCH --job-name=hello-killarney-multinode
...
#SBATCH --output=hello-killarney-multinode.%j.out
#SBATCH --error=hello-killarney-multinode.%j.err
...
# Host name:
echo "Hello Killarney Multinode! I am the sbatch script running on the head node: $(hostname)"
...
for index in $(seq 0 $((SLURM_JOB_NUM_NODES-1))); do
    $SRUN_BIN -lN$index --mem=1G --gres=gpu:1 -c 4 -N 1 -n 1 -r $index --output hello-killarney-multinode.$SLURM_JOB_ID.worker-$index.out bash -c "echo Hello Killarney Multinode Worker! I am running on host:; hostname" &
done
